        ...
        DependencyGraph& graph)
{
    ...
    if (w1 > 0 && graph.has_head(w1, w2))
        return "L(" + graph.get_arc_label(w1, w2) + ")";
    else if (w1 >= 0
            && graph.has_head(w2, w1)
            && !c.has_other_child(w2, graph))
        return "R(" + graph.get_arc_label(w2, w1) + ")";
    else
        return "S";
}
...
bool ArcStandard::can_process(DependencyGraph& graph)
{
    // arc-standard can only process projective trees
